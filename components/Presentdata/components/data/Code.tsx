...
import React, { useState, useId, ReactNode, useRef, useEffect, useCallback, useMemo, memo, isValidElement, ReactElement } from 'react';
import { CheckIcon, CopyIcon, Edit, Settings, TextUnWrapIcon, TextWrapIcon } from '@/components/Presentdata/Icons';
import { CodeSandboxModal } from './Sandbox';
...
export const Tab = memo<TabProps>(({ language, fileName = false, linenumbers, children }) => {
    return (
        <div>
            {children}
        </div>
...
export const Code = memo<CodeProps>(({
    ...
    const defaultLineNumbers = useMemo(() => {
        return childrenArray.some((child) => child.props.linenumbers === true);
    }, [childrenArray]);

    const [isLineNumbers, setIsLineNumbers] = useState(defaultLineNumbers);

    const toggleLineNumbers = () => {
        setIsLineNumbers(e => !e);
    }
    ...
    return (
        <>
            ...
            >
                ...
                {/* Control buttons */}
                <div className="sticky z-10 top-28 max-lg:top-5 w-full flex pointer-events-none">
                        ...
                        <div ref={settingsRef} className="relative w-fit">
                            ...
                            {isSettings && (
                                <div
                                    ...
                                    aria-label="File options"
                                >
                                    {["linenumbers", "sandbox"].map((item, index) => (
                                        <>
                                            {item === "linenumbers" && (<div className={`w-full flex items-center justify-start rounded-lg cursor-pointer hover:bg-background-tertiary`}>
                                                <button
                                                    className={`w-full flex items-center p-1.5 px-2.5 gap-1.5 text-xs font-mono font-medium text-foreground-accent hover:text-foreground cursor-pointer`}
                                                    onClick={toggleLineNumbers}
                                                    onKeyDown={(e) => handleKeyDown(e, index)}
                                                    role="option"
                                                // aria-selected={activeTab === index}
                                                >
                                                    {item === "linenumbers" && ("Line Numbers")}
                                                </button>
                                            </div>)}
                                            {item === "sandbox" && (
                                                <div
                                                    className="w-full flex items-center justify-start rounded-lg cursor-pointer hover:bg-background-tertiary"
                                                >

                                                    <button
                                                        className='flex items-center p-1.5 px-2.5 gap-1.5 text-xs font-mono font-medium w-full text-foreground-accent hover:text-foreground cursor-pointer'
                                                        onClick={() => setSandboxOpen(true)}
                                                    >
                                                        Run in Sandbox
                                                    </button>
